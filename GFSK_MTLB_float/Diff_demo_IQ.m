function  [len, demo_raw_data] = Diff_demo_IQ(GFSKBaseband, raw_data, Insert)
...
for i = Insert+1:Insert:length(GFSKBaseband)
    ...
    diff = 0;
    for k = 1:1
        diff = diff + imag(GFSKBaseband(i-k)) * real(GFSKBaseband(i-Insert+k)) - real(GFSKBaseband(i-k)) * imag(GFSKBaseband(i-Insert+k)); 
